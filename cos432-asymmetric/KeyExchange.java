...
public class KeyExchange {
    ...
    private final BigInteger p, g; // DH constants
    ...
    public KeyExchange(PRGen rand, boolean iAmServer) {
        ...
        this.p = DHConstants.p; this.g = DHConstants.g;

        int len = (p.bitLength() + 7) / 8;
        byte[] xb = new byte[len];
        rand.nextBytes(xb);

        BigInteger tmp = HW2Util.bytesToBigInteger(xb);
        this.x = tmp.mod(p.subtract(BigInteger.valueOf(3))).add(BigInteger.valueOf(2)); // [2, p-2]
        this.X = g.modPow(x, p);

        this.outMsg = HW2Util.bigIntegerToBytes(X, (p.bitLength() + 7) / 8);
    }
    ...
    public byte[] processInMessage(byte[] inMessage) {
        ...
        BigInteger Y = HW2Util.bytesToBigInteger(inMessage);
        ...
        byte[] sBytes = HW2Util.bigIntegerToBytes(S, (p.bitLength() + 7) / 8);
